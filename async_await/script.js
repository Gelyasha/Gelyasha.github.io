...
const findCity = async (city) => {
    try {
        const url = `${SERVER_URL}?q=${city}&appid=${API_KEY}`;
        let savingCity = {};
        const responseCityData = await fetch(url);
        ...
        const url_p = `${FORECAST_URL}?q=${city}&appid=${API_KEY}`;
        const responseForecastData = await fetch(url_p);
        const anotherData = await responseForecastData.json();
        const { list } = anotherData;
        const [date1, date2, date3] = list;
        updateDate(forecast1, date1)
        updateDate(forecast2, date2)
        updateDate(forecast3, date3)
